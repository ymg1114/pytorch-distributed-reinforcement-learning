...
def reset_shared_memory(args, obs_shape):
    shm_ref = {}  # Learner / LearnerStorage 에서 공유할 메모리 주소를 담음

    obs_batch = np.zeros(
        args.seq_len * args.batch_size * mul(obs_shape),
        dtype=np.float32,
    )  # observation-space
    set_shared_memory(shm_ref, obs_batch, "obs_batch")

    act_batch = np.zeros(
        args.seq_len * args.batch_size * 1, dtype=np.float32
    )  # not one-hot, but action index (scalar)
    set_shared_memory(shm_ref, act_batch, "act_batch")

    rew_batch = np.zeros(args.seq_len * args.batch_size * 1, dtype=np.float32)  # scalar
    set_shared_memory(shm_ref, rew_batch, "rew_batch")

    logits_batch = np.zeros(
        args.seq_len * args.batch_size * args.action_space,
        dtype=np.float32,
    )  # action-space (logits)
