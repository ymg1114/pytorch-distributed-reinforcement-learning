        ...
        while True:
            ...
            while True:
                if seq_num+self.args.seq_len+1 > len(data[0]):
                    ...
                    self.workers_obs[:, self.batch_num] = torch.cat( obs_list + [ obs_list[-1] ] * (diff+1), dim=0 )
                    self.workers_actions[:, self.batch_num] = torch.cat( actions_list + [ actions_list[-1] ] * (diff+1), dim=0 )
                    self.workers_rewards[:, self.batch_num] = torch.cat( rewards_list + [ rewards_list[-1] ] * (diff+1), dim=0 )
                    self.workers_log_probs[:, self.batch_num] = torch.cat( log_probs_list + [ log_probs_list[-1] ] * (diff+1), dim=0 )
                    self.workers_masks[:, self.batch_num] = torch.cat( masks_list + [ masks_list[-1] ] * (diff+1), dim=0 )
