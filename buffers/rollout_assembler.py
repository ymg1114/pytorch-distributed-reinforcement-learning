...
class RolloutAssembler:
    ...
    async def push(self, data):
        ...
        self.roll_q = {id: tj for id, tj in self.roll_q.items() if time.monotonic() - tj.gen_time < 0.5}
        self.roll_q_done = {id: tj for id, tj in self.roll_q_done.items() if time.monotonic() - tj.gen_time < 0.5}
        ...
        if _id in self.roll_q:  # 기존 경기에 데이터 추가
            ...
            if len(self.roll_q_done) > 0:
                ...
                tj_ = Trajectory2(self.seq_len, time.monotonic())  # Trajectory 객체 생성을 통한 할당
            tj_.put(data)
            self.roll_q[_id] = tj_
