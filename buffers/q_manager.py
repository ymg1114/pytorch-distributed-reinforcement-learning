...
class QManager():
    ...
    def make_batch(self):
        while True:
            ...
            while True:
                if self.seq_num+self.seq_len+1 > len(data[0]):
                    diff = self.seq_len - (len(data[0]) - self.seq_num)

                    self.workers_obs[:, self.batch_num] = torch.cat( ( data[0][self.seq_num: ] + (diff+1)*list( data[0][self.seq_num: ][-1]) ), dim=0 )
                    self.workers_actions[:, self.batch_num] = torch.cat( ( data[1][self.seq_num: ] + (diff+1)*list( data[1][self.seq_num: ][-1]) ), dim=0 )
                    self.workers_rewards[:, self.batch_num] = torch.cat( ( data[2][self.seq_num: ] + (diff+1)*list( data[2][self.seq_num: ][-1]) ), dim=0 )
                    self.workers_log_probs[:, self.batch_num] = torch.cat( ( data[3][self.seq_num: ] + (diff+1)*list( data[3][self.seq_num: ][-1]) ), dim=0 )
                    self.workers_masks[:, self.batch_num] = torch.cat( ( data[4][self.seq_num: ] + (diff+1)*list( data[4][self.seq_num: ][-1]) ), dim=0 )
                    ...
                    self.workers_obs[:, self.batch_num] = torch.cat( data[0][self.seq_num: self.seq_num+self.seq_len+1], dim=0 )
                    self.workers_actions[:, self.batch_num] = torch.cat( data[1][self.seq_num: self.seq_num+self.seq_len+1], dim=0 )
                    self.workers_rewards[:, self.batch_num] = torch.cat( data[2][self.seq_num: self.seq_num+self.seq_len+1], dim=0 )
                    self.workers_log_probs[:, self.batch_num] = torch.cat( data[3][self.seq_num: self.seq_num+self.seq_len+1], dim=0 )
                    self.workers_masks[:, self.batch_num] = torch.cat( data[4][self.seq_num: self.seq_num+self.seq_len+1], dim=0 )
